package com.cyq7on.tranceroutlib.service;
...
public class LDNetTraceRoute {
    ...
    public static LDNetTraceRoute newInstance() {
        return new LDNetTraceRoute();
    }
    ...
    public void startTraceRoute(String host, int hop) {
        if (isCTrace && loaded) {
            try {
                startJNICTraceRoute(host);
            } catch (UnsatisfiedLinkError e) {
                ...
                // 如果c调用失败改调JAVA代码
                Log.i("LDNetTraceRoute", "调用java模拟traceRoute");
                TraceTask trace = new TraceTask(host, hop);
                execTrace(trace);
            }
        } else {
            TraceTask trace = new TraceTask(host, hop);
    ...
    public void startTraceRoute(String host, boolean isNode) {
        if (isCTrace && loaded) {
            try {
                startJNICTraceRoute(host);
            } catch (UnsatisfiedLinkError e) {
                ...
                // 如果c调用失败改调JAVA代码
                Log.i("LDNetTraceRoute", "调用java模拟traceRoute");
                TraceTask trace = new TraceTask(host, 1, isNode);
                execTrace(trace);
            }
        } else {
            TraceTask trace = new TraceTask(host, 1, isNode);
    ...
    private void execTrace(TraceTask trace) {
        ...
        Log.d(LOG_TAG, "TraceTask hop:" + trace.getHop());
        try {
            ...
            while (!finish) {
                ...
                if (m.find()) {
                    ...
                    if (status.length() == 0) {
                        ...
                        finish = true;
                        Log.d(LOG_TAG, "unknown host or network error\n");
                    } else {
                        ...
                        if (matcherTime.find()) {
                            ...
                            /*if (!trace.isNode()) {
                                LDNetTraceRoute traceRouter = LDNetTraceRoute.newInstance();
                                traceRouter.initListenter(new LDNetTraceRouteListener() {
                                    @Override
                                    public void OnNetTraceUpdated(String log) {
                                        Log.d(LOG_TAG, "node:" + pingIp + "->" + log);
                                    }

                                    @Override
                                    public void OnNetTraceFinished() {
                                        Log.d(LOG_TAG, "node:" + pingIp + "->" + "finish");
                                    }
                                });
                                traceRouter.isCTrace = false;
                                traceRouter.startTraceRoute(pingIp, true);
                                Log.d(LOG_TAG, "startTraceRoute");
                            }*/
                        } else {
                            log.append(trace.getHop());
                            log.append(":");
                ...
                // 否则：what
                    ...
                    if (matchPingIp.find()) {
                        ...
                        if (matcherTime.find()) {
                            ...
                            listener.OnNetTraceUpdated(log.toString());
                            Log.d(LOG_TAG, log.toString());
//                            Log.d(LOG_TAG, trace.getHop() + " " + pingIp + " " + "timeout");
                        }
                        finish = true;
                    } else {
                        if (str.length() == 0) {
    ...
    private class TraceTask {
        ...
        public TraceTask(String host, int hop, boolean isNode) {
            super();
            this.host = host;
            this.hop = hop;
            this.isNode = isNode;
